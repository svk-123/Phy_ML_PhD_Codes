...
for p in range(9,11):
    ...
    for i in range(len(tmp2)):
        ...
        plt.savefig('./plot2/up_%s.eps'%i, format='eps')
        #plt.show() 
        plt.close()
        ...
        #patch.set_facecolor('black')
        plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
        plt.savefig('./plot2/lr_%s.eps'%i, format='eps')
        #plt.show() 
        plt.close() 
        
        img_up = io.imread('./plot2/up_%s.eps'%i, as_grey=True)  # load the image as grayscale
        ...
        img_lr = io.imread('./plot2/lr_%s.eps'%i, as_grey=True)  # load the image as grayscale
