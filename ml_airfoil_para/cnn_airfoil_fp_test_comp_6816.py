...
I=np.random.randint(0,len(out),30)

xtr1=xtr1[I]
ttr1=ttr1[I]
name=name[I]

model_6=load_model('./selected_model/case_1_p6_tanh/model_cnn/final_cnn.hdf5') 
model_8=load_model('./selected_model/case_1_p8_tanh/model_cnn/final_cnn.hdf5') 
model_16=load_model('./selected_model/case_1_p16_tanh/model_cnn/final_cnn.hdf5') 
...
for i in range(len(c6)):
    ...
    plt.plot(xx,c6[i,:35]*0.25,'r',lw=2,label='CNN P-6')
    plt.plot(xx,c6[i,35:]*0.25,'r',lw=2)
    
    plt.plot(xx,c8[i,:35]*0.25,'g',lw=2,label='CNN P-8')
    plt.plot(xx,c8[i,35:]*0.25,'g',lw=2)    
    
    plt.plot(xx,c16[i,:35]*0.25,'b',lw=2,label='CNN P-16')
    plt.plot(xx,c16[i,35:]*0.25,'b',lw=2)
    
    plt.plot(xx,ttr1[i,:35],'k',marker='o', mfc='None',mew=1.0,ms=8,lw=0,label='True')
