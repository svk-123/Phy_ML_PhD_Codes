...
with open(path + 'hist_c3_f2.pkl', 'rb') as infile:
    result = pickle.load(infile)
h1=result[0]
h1l=h1['loss']
h1vl=h1['val_loss']
h1l=np.asarray(h1l)/0.92
h1vl=np.asarray(h1vl)/0.92


with open(path + 'hist_c4_f2.pkl', 'rb') as infile:
    result = pickle.load(infile)
h2=result[0]
h2l=h2['loss']
h2vl=h2['val_loss']
h2l=np.asarray(h2l)/0.96
h2vl=np.asarray(h2vl)/1.1

with open(path + 'hist_c5_f2.pkl', 'rb') as infile:
    result = pickle.load(infile)
h3=result[0]
h3l=h3['loss']
h3vl=h3['val_loss']
...
with open(path + 'hist_c5_f3.pkl', 'rb') as infile:
    result = pickle.load(infile)
h4=result[0]
h4l=h4['loss']
h4vl=h4['val_loss']
...
with open(path + 'hist_c5_f4.pkl', 'rb') as infile:
    result = pickle.load(infile)
h5=result[0]
h5l=h5['loss']
h5vl=h5['val_loss']
...
plt.xlim([-50,2000])
#plt.ylim([-0.2,0.2])    
plt.savefig('fc.tiff', format='tiff', bbox_inches='tight',dpi=300)
plt.show()
