...
with tf.Session() as sess:
    ...
    path='./tf_model/'
    new_saver = tf.train.import_meta_graph(path + 'model_1.meta' )
    new_saver.restore(sess, tf.train.latest_checkpoint(path))
    
       
    tf_dict = {'input:0': my_inp}
    para_to_load = graph.get_tensor_by_name('para/BiasAdd:0')   
...
#    
#    plt.plot(xx[::-1],tout[k][0:35],'o',lw=3,label='CNN')
#    plt.plot(xx,tout[k][35:],'o',lw=3)
#    
#    plt.xlim([-0.05,1.05])
#    plt.ylim([-0.25,0.25])
