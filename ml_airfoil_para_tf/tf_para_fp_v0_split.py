...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y,rst=False):
        ...
        self.y_inter = tf.placeholder(tf.float32, shape=[None, 8],name='output1')        
        
        self.y_tf = tf.placeholder(tf.float32, shape=[None, self.y.shape[1]],name='output2')

        self.y_inter = self.net_NS1(self.x_tf)   
        
        self.y_pred = self.net_NS2(self.y_inter)
    ...
    def neural_net2(self, XX):

       
        self.fc5= tf.layers.dense(XX, 100, activation=tf.nn.tanh, name='input_para')        
        fc6= tf.layers.dense(self.fc5, 100, activation=tf.nn.tanh)
