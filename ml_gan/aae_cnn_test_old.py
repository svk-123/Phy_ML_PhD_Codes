...
path='./models/case_2_aae_uiuc/saved_model/'
iters=99000

# load json and create model
json_file = open(path+'aae_decoder.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
decoder = model_from_json(loaded_model_json)
# load weights into new model
decoder.load_weights(path+"aae_decoder_weights.hdf5")
print("Loaded model from disk")  


# load json and create model
json_file = open(path+'aae_encoder.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
encoder = model_from_json(loaded_model_json)
# load weights into new model
encoder.load_weights(path+"aae_encoder_weights.hdf5")
print("Loaded model from disk")  

 # load json and create model
json_file = open(path+'aae_discriminator.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
discriminator = model_from_json(loaded_model_json)
# load weights into new model
discriminator.load_weights(path+"aae_discriminator_weights.hdf5")
print("Loaded model from disk")  
...
data_file='./data_file/foil_param.pkl'	
...
for k in range(0):
        ...
        l=latent[I]
        noise=abs(np.random.normal(0,0.1,8))
