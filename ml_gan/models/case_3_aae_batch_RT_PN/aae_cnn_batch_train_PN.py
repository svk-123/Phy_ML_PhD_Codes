    ...
    def train(self, epochs, batch_size=32, sample_interval=50):
        ...
        for epoch in range(epochs):
            for batch in range(nbatches-1):
                ...
                idx = (np.linspace(batch*batch_size,(batch+1)*batch_size-1,batch_size)).astype(int)
                imgs = X_train[idx]
                out  = Y_train[idx]
                ...
                latent_fake[:,0:4]=latent_fake[:,0:4]*-1.0
                ...
                # Plot the progress
                print ("%d %d [D loss: %f, acc: %.2f%%] [G loss: %f, mse: %f], lr=%e" % (epoch, batch, d_loss[0], 100*d_loss[1], g_loss[0], g_loss[1],\
                       self.my_lr))
                ...
                if ((epoch+1) % 300 == 0 and batch == nbatches-2 and self.my_lr > 1.e-6):
                    self.my_lr=self.my_lr*0.5
    
                if ((epoch+1) % 300 == 0 and batch == nbatches-2):                
...
if __name__ == '__main__':
    ...
    aae.train(epochs=5000, batch_size=64, sample_interval=100)
