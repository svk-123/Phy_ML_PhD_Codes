...
for i in range(1):
    ...
    plt.plot(data[i][:L,0], data[i][:L,2] + data[i][:L,3],'%s'%c[i],marker='None',mfc='r',ms=12,lw=2,markevery=l1,label='MSE')
    plt.plot(data[i][:L,0], data[i][:L,4] ,'%s'%c[i+1],marker='None',mfc='r',ms=12,lw=2,markevery=l1,label='Gov. Res')
