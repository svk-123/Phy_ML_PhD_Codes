...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, p, u, v, \
        ...
        #MSE wall
        self.u_iw_pred, self.v_iw_pred, _, self.dp_pred = self.net_NS1(self.x_iw_tf, self.y_iw_tf,self.nx_iw_tf, self.ny_iw_tf)
        
        #MSE outlet
        _, _, self.p_oo_pred, self.du_pred = self.net_NS2(self.x_oo_tf, self.y_oo_tf,self.nx_oo_tf, self.ny_oo_tf) 
...
#                    tf.reduce_mean(tf.square(self.p_tf - self.p_pred)) 
        ...
        self.loss_2 =  tf.reduce_mean(tf.square(self.dp_pred))
                                                            
        self.loss_3 = tf.reduce_mean(tf.square(self.p_oo_tf - self.p_oo_pred)) 

        self.loss_4 = tf.reduce_mean(tf.square(self.du_pred))                        
    ...
    def callback(self, loss, loss_1, loss_2, loss_3, loss_4, loss_5):
    ...
    def train(self, nIter, lbfgs=False): 
        ...
        while(count < nIter):
            ...
            for i in range(1):
                ...
                tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.u_tf: self.u, self.v_tf: self.v, self.p_tf: self.p, \
                           ...
                           self.nx_iw_tf: self.nx_iw, self.ny_iw_tf: self.ny_iw, self.nx_oo_tf: self.nx_oo, self.ny_oo_tf:self.ny_oo,\
                           self.tf_lr:lr, self.xg_tf: self.xg, self.yg_tf: self.yg}
            
                _,loss_value,lv_1,lv_2,lv_3,lv_4,lv_5=self.sess.run([self.train_op_Adam,self.loss,self.loss_1,self.loss_2,self.loss_3,self.loss_4,self.loss_5], tf_dict)
            ...
            print('It: %d, Loss: %.6e, Loss-1:%0.6e, Loss-2:%0.6e, Loss-3:%0.6e, Loss-4:%0.6e, Loss-5:%0.6e,\
                  lr:%0.6f, Time: %.2f \n' \
                          %(count, avg_loss, avg_lv_1, avg_lv_2, avg_lv_3, avg_lv_4, avg_lv_5, lr, elapsed))
        ...
        if (lbfgs==True):
            ...
            self.optimizer.minimize(self.sess,
                                    feed_dict = tf_dict,
                                    fetches = [self.loss,self.loss_1,self.loss_2,self.loss_3,self.loss_4,self.loss_5],
