...
#    v_train = np.concatenate((xyu_inlet[:,4:5],xyu_outlet_t[:,4:5],xyu_outlet_r[:,4:5],xyu_s[:,4:5]),axis=0)  
    ...
    #pg_train = np.concatenate((xyu_inlet[:,2:3],xyu_wall[:,2:3],xyu_outlet_t[:,2:3],xyu_outlet_r[:,2:3],xyu_int[:,2:3]),axis=0)
    
#    #plot
#    plt.hist(pg_train,bins=100)
#    plt.title('original')
#    plt.show()
...
#    pmin= -0.001616
#    
#    pg_tr_mm=pg_train.copy()
#    
#    for i in range(len(pg_train)):
#        pg_tr_mm[i] = ((maxi-mini)/(pmax-pmin))*(pg_train[i]-pmax) + maxi
#            
#    plt.hist(pg_tr_mm,bins=100)
#    plt.title('minmax')
#    plt.show()
#    
#    pg_tr_rec=pg_train.copy()
#    for i in range(len(pg_train)):
#        pg_tr_rec[i] = ((pmax-pmin)/(maxi-mini))*(pg_tr_mm[i]-maxi) + pmax    
#
#    plt.hist(pg_tr_rec,bins=100)
#    plt.title('rec')
#    plt.show()
    ...
    #nd=boxcox(pg_tr_mm,-4)
    
    pg_train_1=pg_train+0.001616+1e-16
    la=0.1469735
    nd=boxcox(pg_train_1,la)
    
    plt.hist(nd,bins=100)
    plt.title('boxcox')
    plt.show()

    nd_rec=inv_boxcox(nd,la)
    
    plt.hist(nd_rec,bins=100)
    plt.title('inv_boxcox')
    plt.show()        
    '''
