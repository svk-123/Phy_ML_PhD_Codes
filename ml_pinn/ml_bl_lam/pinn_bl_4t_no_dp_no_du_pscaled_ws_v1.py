...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, p, u, v, \
        ...
        self.nu = tf.constant([1/5000.], dtype=tf.float32)
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
    ...
    def net_NS1(self, x, y, nx, ny):
        ...
        p = ((self.pmax-self.pmin)/(self.maxi-self.mini))*(p-self.maxi) + self.pmax    
...
if __name__ == "__main__": 
    ...
    #sampling
    xyu_s=np.loadtxt(path + 'bl_sample_x30_20.dat',skiprows=1)
    ...
    for i in range(len(p_inlet)):
        p_inlet[i] = ((maxi-mini)/(pmax-pmin))*(p_inlet[i]-pmax) + maxi
        
    for i in range(len(p_wall)):
        p_wall[i] = ((maxi-mini)/(pmax-pmin))*(p_wall[i]-pmax) + maxi
        
    for i in range(len(p_outlet_t)):
        p_outlet_t[i] = ((maxi-mini)/(pmax-pmin))*(p_outlet_t[i]-pmax) + maxi    
    
    for i in range(len(p_outlet_r)):
        p_outlet_r[i] = ((maxi-mini)/(pmax-pmin))*(p_outlet_r[i]-pmax) + maxi        

    for i in range(len(p_iw)):
        p_iw[i] = ((maxi-mini)/(pmax-pmin))*(p_iw[i]-pmax) + maxi       

    for i in range(len(p_oo)):
        p_oo[i] = ((maxi-mini)/(pmax-pmin))*(p_oo[i]-pmax) + maxi 

    for i in range(len(p_s)):
        p_s[i] = ((maxi-mini)/(pmax-pmin))*(p_s[i]-pmax) + maxi 

    for i in range(len(p_train)):
        p_train[i] = ((maxi-mini)/(pmax-pmin))*(p_train[i]-pmax) + maxi     
