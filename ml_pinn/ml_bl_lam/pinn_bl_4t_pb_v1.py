...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, p, u, v, \
        ...
        self.x_iw_tf = tf.placeholder(tf.float32, shape=[None, self.x_iw.shape[1]])
        self.y_iw_tf = tf.placeholder(tf.float32, shape=[None, self.y_iw.shape[1]])
        ...
        self.nx_iw_tf = tf.placeholder(tf.float32, shape=[None, self.nx_iw.shape[1]])         
        self.ny_iw_tf = tf.placeholder(tf.float32, shape=[None, self.ny_iw.shape[1]])   
...
if __name__ == "__main__": 
    ...
    idx = np.random.choice(len(xyu_s), 1000, replace=False)
    x_s = xyu_s[idx,0:1]
    y_s = xyu_s[idx,1:2]
    p_s = xyu_s[idx,2:3]
    u_s = xyu_s[idx,3:4]
...
plt0, =plt.plot(x_s,y_s,'og',linewidth=0,ms=3,label='MSE internal pts: 1000 ',zorder=2)
