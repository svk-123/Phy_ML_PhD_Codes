...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, p, u, v, \
                 x_i, y_i, p_i, u_i, v_i, nx_i, ny_i, \
                 x_w, y_w, p_w, u_w, v_w, nx_w, ny_w, \
        ...
        self.nu = tf.constant([0.0002], dtype=tf.float32)
        ...
        self.x_i_tf = tf.placeholder(tf.float32, shape=[None, self.x_i.shape[1]], name='input1a')
        self.y_i_tf = tf.placeholder(tf.float32, shape=[None, self.y_i.shape[1]], name='input1b')
        self.u_i_tf = tf.placeholder(tf.float32, shape=[None, self.u_i.shape[1]])
        self.v_i_tf = tf.placeholder(tf.float32, shape=[None, self.v_i.shape[1]]) 
        self.nx_i_tf = tf.placeholder(tf.float32, shape=[None, self.nx_i.shape[1]], name='input1c')         
        self.ny_i_tf = tf.placeholder(tf.float32, shape=[None, self.ny_i.shape[1]], name='input1d')   

        self.x_w_tf = tf.placeholder(tf.float32, shape=[None, self.x_w.shape[1]], name='input2a')
        self.y_w_tf = tf.placeholder(tf.float32, shape=[None, self.y_w.shape[1]], name='input2b')
        self.u_w_tf = tf.placeholder(tf.float32, shape=[None, self.u_w.shape[1]])
        self.v_w_tf = tf.placeholder(tf.float32, shape=[None, self.v_w.shape[1]]) 
        self.nx_w_tf = tf.placeholder(tf.float32, shape=[None, self.nx_w.shape[1]], name='input2c')         
        self.ny_w_tf = tf.placeholder(tf.float32, shape=[None, self.ny_w.shape[1]], name='input2d')  
        ...
        #MSE inlet
        self.u_i_pred, self.v_i_pred, _, self.dp_i_pred = self.net_NS1(self.x_i_tf, self.y_i_tf,self.nx_i_tf, self.ny_i_tf)
        
        #MSE wall
        self.u_w_pred, self.v_w_pred, _, self.dp_w_pred = self.net_NS1a(self.x_w_tf, self.y_w_tf,self.nx_w_tf, self.ny_w_tf)   
        
        #MSE outlet
        _, _, self.p_oo_pred, self.du_oo_pred = self.net_NS2(self.x_oo_tf, self.y_oo_tf,self.nx_oo_tf, self.ny_oo_tf) 
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
        ...
        #inlet    
        self.loss_1 = tf.reduce_mean(tf.square(self.u_i_tf - self.u_i_pred)) + \
                        tf.reduce_mean(tf.square(self.v_i_tf - self.v_i_pred)) 
                        #tf.reduce_mean(tf.square(self.dp_i_pred))
                        
        #wall                
        self.loss_2 = tf.reduce_mean(tf.square(self.u_w_tf - self.u_w_pred)) + \
                        tf.reduce_mean(tf.square(self.v_w_tf - self.v_w_pred)) 
                        #tf.reduce_mean(tf.square(self.dp_w_pred))                                                            
        
        #outlet
        self.loss_3 = tf.reduce_mean(tf.square(self.p_oo_tf - self.p_oo_pred)) + \
                      tf.reduce_mean(tf.square(self.du_oo_pred))
        
        self.loss_4 = tf.reduce_mean(tf.square(self.f_c_pred)) + \
                    tf.reduce_mean(tf.square(self.f_u_pred)) + \
                    tf.reduce_mean(tf.square(self.f_v_pred))
                    
        self.loss = self.loss_0 + self.loss_1 + self.loss_2 + self.loss_3 + self.loss_4
    ...
    def net_NS2(self, x, y, nx, ny):
        ...
        dn  = nx*(u_x*nx + v_x*ny) + ny*(u_y*nx + v_y*ny)
    ...
    def callback(self, loss, loss_0,loss_1, loss_2, loss_3,loss_4):
        print('Loss: %.6e %.6e %.6e %.6e %.6e %.6e \n' % (loss,loss_0,loss_1,loss_2, loss_3,loss_4))       
        self.fp.write('00, %.6e, %.6e, %.6e, %.6e, %.6e, %.6e \n'% (loss,loss_0,loss_1,loss_2, loss_3,loss_4)) 
    ...
    def train(self, nIter, lbfgs=False): 
        ...
        while(count < nIter):
            ...
            for i in range(1):
                ...
                tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.u_tf: self.u, self.v_tf: self.v, self.p_tf: self.p, \
                           self.x_i_tf: self.x_i, self.y_i_tf: self.y_i, self.u_i_tf: self.u_i, self.v_i_tf: self.v_i, \
                           self.x_w_tf: self.x_w, self.y_w_tf: self.y_w, self.u_w_tf: self.u_w, self.v_w_tf: self.v_w, \
                           self.x_oo_tf: self.x_oo, self.y_oo_tf: self.y_oo, self.p_oo_tf: self.p_oo,\
                           self.nx_i_tf: self.nx_i, self.ny_i_tf: self.ny_i,\
                           self.nx_w_tf: self.nx_w, self.ny_w_tf: self.ny_w,\
                           self.nx_oo_tf: self.nx_oo, self.ny_oo_tf:self.ny_oo,\
                           self.tf_lr:lr, self.xg_tf: self.xg, self.yg_tf: self.yg}
            
                _,loss_value,lv_0,lv_1,lv_2,lv_3,lv_4=self.sess.run([self.train_op_Adam,self.loss,self.loss_0,self.loss_1,self.loss_2,self.loss_3,self.loss_4], tf_dict)
            ...
            print('It: %d, Loss: %.6e, Loss-0:%0.6e, Loss-1:%0.6e, Loss-2:%0.6e, Loss-3:%0.6e, Loss-4:%0.6e, lr:%0.6f, Time: %.2f \n' \
                          %(count, avg_loss,avg_lv_0,avg_lv_1, avg_lv_2, avg_lv_3, avg_lv_4, lr, elapsed))
            
            self.fp.write('%d, %.6e, %0.6e, %0.6e, %0.6e, %0.6e, %0.6e, %0.6e, %.2f \n' \
                          %(count, avg_loss, avg_lv_0, avg_lv_1, avg_lv_2, avg_lv_3, avg_lv_4, lr, elapsed))    
            start_time = time.time()
        ...
        if (lbfgs==True):
            ...
            self.optimizer.minimize(self.sess,
                                    feed_dict = tf_dict,
                                    fetches = [self.loss,self.loss_0,self.loss_1,self.loss_2,self.loss_3,self.loss_4],
