...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, r, a, u, v, p):
        ...
        # Initialize parameters (1/2500)
        self.nu = tf.constant([0.0004], dtype=tf.float32)
        ...
        self.a_tf = tf.placeholder(tf.float32, shape=[None, self.r.shape[1]],name='input3')
        ...
        self.u_pred, self.v_pred, self.p_pred, self.f_c_pred, self.f_u_pred, self.f_v_pred = self.net_NS(self.x_tf, self.y_tf, self.r_tf, self.a_tf)
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
    ...
    def net_NS(self, x, y, r, a):

        uvp = self.neural_net(tf.concat([x,y,r,a], 1))
    ...
    def train(self, nIter, lbfgs=False): 
        ...
        while(count < nIter):
            ...
            for i in range(total_batch):
                ...
                batch_x, batch_y, batch_r, batch_a, batch_u, batch_v, batch_p = self.get_batch(i,batch_size,total_batch)
                
                tf_dict = {self.x_tf: batch_x, self.y_tf: batch_y, self.r_tf: batch_r, self.a_tf: batch_a,
...
if __name__ == "__main__": 
    ...
    reytmp=np.asarray(reytmp)/2500.    
    aoatmp=np.asarray(aoatmp)/16.
    ...
    # Training
    model = PhysicsInformedNN(x_train, y_train, r_train, aoa_train, u_train, v_train, p_train)
