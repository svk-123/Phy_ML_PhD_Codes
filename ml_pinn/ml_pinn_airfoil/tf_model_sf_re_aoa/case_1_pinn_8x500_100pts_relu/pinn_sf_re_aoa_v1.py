...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, r, a, para, u, v, p, rst=False):
        ...
        self.u_pred, self.v_pred, self.p_pred, self.f_c_pred, self.f_u_pred, self.f_v_pred = self.net_NS(self.x_tf,\
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
    ...
    def neural_net(self, X):
        ...
        l1 = tf.layers.dense(l1, 500, activation=tf.nn.relu)
        l1 = tf.layers.dense(l1, 500, activation=tf.nn.relu)
        l1 = tf.layers.dense(l1, 500, activation=tf.nn.relu)
        l1 = tf.layers.dense(l1, 500, activation=tf.nn.relu)
        l1 = tf.layers.dense(l1, 500, activation=tf.nn.relu)
    ...
    def train(self, nIter, lbfgs=False): 
        ...
        if (lbfgs==True):
            ...
            tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.r_tf: self.r, self.a_tf: self.a, self.para_tf: self.para,
...
if __name__ == "__main__": 
    ...
    for ii in range(1):
        #x,y,Re,u,v
        with open('../../ml_para_sf/data_file/foil_nacan_comb_pinn_300pts_tr.pkl', 'rb') as infile:
    ...
    # Training
    model = PhysicsInformedNN(x_train, y_train, r_train, aoa_train, para_train, u_train, v_train, p_train, False)
