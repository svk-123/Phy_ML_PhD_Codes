...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, u, v, p, x_vb, y_vb, u_vb, v_vb, x_hb, y_hb, u_hb, v_hb, xg, yg):
        ...
        self.u_vb_pred, self.v_vb_pred, _, self.p_x, _ = self.net_NS2A(self.x_vb_tf, self.y_vb_tf)
        
        self.u_hb_pred, self.v_hb_pred, _, _, self.p_y = self.net_NS2B(self.x_hb_tf, self.y_hb_tf)        
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
        ...
        self.loss_2 = tf.reduce_mean(tf.square(self.u_vb_tf - self.u_vb_pred)) + \
                    ...
                    tf.reduce_mean(abs(self.p_x) + abs(self.p_y))
