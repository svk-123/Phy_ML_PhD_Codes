...
Re=100
path='./tf_model/case_1_naca4412_Re100_aoa20_nodp_nodv_ws_ar_30x2/tf_model/'
suff='ws_f3'
data1=[]
for i in range(1):
...
#    plt.plot(len(data2[0]), data2[i][:][1]-data2[i][:][3] ,'%s'%c[0],marker='None',mfc='r',ms=12,lw=2,markevery=l1)
#    plt.plot(len(data2[0]), data2[i][:][3]                ,'%s'%c[2],marker='None',mfc='r',ms=12,lw=2,markevery=l1)
...
#plt.xlim([-50,6000])
#plt.ylim([5e-6,1e-3])    
plt.savefig('./plot/bfs_%s.tiff'%suff, format='tiff', bbox_inches='tight',dpi=300)
plt.show()
#--------------------