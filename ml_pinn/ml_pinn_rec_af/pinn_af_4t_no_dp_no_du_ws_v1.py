...
if __name__ == "__main__": 
    ...
    #import wall bc
    #x,y,p,u,v
    xyu_inlet=np.loadtxt(path + 'naca4412_inlet3_1211.dat',skiprows=1)
    ...
    xyu_wall=np.loadtxt(path + 'naca4412_wall_300.dat',skiprows=1)
    ...
    xyu_outlet_t=np.loadtxt(path + 'naca4412_outlet1_1211.dat',skiprows=1)
    ...
    nx_outlet_t= xyu_outlet_t[:,4:5]
    ny_outlet_t= xyu_outlet_t[:,4:5]
    ...
    p_oo  = xyu_outlet_t[:,2:3]
    u_oo  = xyu_outlet_t[:,3:4]   
    v_oo  = xyu_outlet_t[:,4:5]
    nx_oo = xyu_outlet_t[:,5:6]   
    ny_oo = xyu_outlet_t[:,6:7]

    #sampling
    xyu_s=np.loadtxt(path + 'naca4412_sample_ar_30x2.dat',skiprows=1)
...
#    v_train = np.concatenate((xyu_inlet[:,4:5],xyu_outlet_t[:,4:5],xyu_outlet_r[:,4:5],xyu_s[:,4:5]),axis=0)  
    ...
    xyu_int=np.loadtxt(path + 'naca4412_internal_1211.dat',skiprows=1)    
                
    # internal points with wall BC
    xg_train = np.concatenate((xyu_inlet[:,0:1],xyu_wall[:,0:1],xyu_outlet_t[:,0:1],xyu_int[:,0:1]),axis=0)
    yg_train = np.concatenate((xyu_inlet[:,1:2],xyu_wall[:,1:2],xyu_outlet_t[:,1:2],xyu_int[:,1:2]),axis=0)
...
plt0, =plt.plot(xyu_s[:,0:1],xyu_s[:,1:2],'og',linewidth=0,ms=4,label='MSE internal pts: 60 ',zorder=8)
