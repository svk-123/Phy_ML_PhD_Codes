...
if __name__ == "__main__": 
    ...
    u_wall = xyu_wall[:,3:4]
    v_wall = xyu_wall[:,4:5]   
    nx_wall= xyu_wall[:,4:5]
    ny_wall= xyu_wall[:,4:5]
    ...
    #sampling
    xyu_s=np.loadtxt(path + 'af_internal_21p511.dat',skiprows=1)
    
    idx = np.random.choice(len(xyu_s), 500, replace=False)
    x_s = xyu_s[idx,0:1]
    y_s = xyu_s[idx,1:2]
    p_s = xyu_s[idx,2:3]
    u_s = xyu_s[idx,3:4]
...
#    # Training
#    model = PhysicsInformedNN(x_train, y_train, u_train, v_train, p_train, x_wall, y_wall, u_wall, v_wall, xg_train, yg_train, False)
# 
...
plt.figure(figsize=(6, 5), dpi=100)
plt0, =plt.plot(xyu_inlet[:,0:1],xyu_inlet[:,1:2],'ok',linewidth=0,ms=3,label='MSE BC pts-400 ',zorder=5)
plt0, =plt.plot(xyu_outlet[:,0:1],xyu_outlet[:,1:2],'ok',linewidth=0,ms=3,zorder=5)
plt0, =plt.plot(xyu_top[:,0:1],xyu_top[:,1:2],'ok',linewidth=0,ms=3,zorder=5)
plt0, =plt.plot(xyu_bottom[:,0:1],xyu_bottom[:,1:2],'ok',linewidth=0,ms=3,zorder=5)
plt0, =plt.plot(xyu_wall[:,0:1],xyu_wall[:,1:2],'ok',linewidth=0,ms=3,zorder=5)
plt0, =plt.plot(xg_train,yg_train,'+r',linewidth=0,ms=2,label='Gov Eq. pts-12000 (Residual)',zorder=0)

plt0, =plt.plot(x_s,y_s,'og',linewidth=0,ms=3,label='MSE pts-300 (Sampling)',zorder=10)
...
#plt.xlim(-1,2)
#plt.ylim(-1,1)    
plt.savefig('./plot/mesh3.png', format='png',bbox_inches='tight', dpi=100)
plt.show()
