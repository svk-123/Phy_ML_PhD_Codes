...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, t, p, u, v, \
        ...
        self.nu = tf.constant([1./200.], dtype=tf.float32)
...
if __name__ == "__main__": 
    ...
    path='./data_file/naca0012_200_30_1211_t_24_25_dt_0p05_nt_21/'  
    #import wall bc
    #x,y,p,u,v
    xyu_inlet=np.loadtxt(path + 'naca0012_inlet_1211.dat',skiprows=1)
    ...
    xyu_wall=np.loadtxt(path + 'naca0012_wall_300.dat',skiprows=1)
    ...
    xyu_outlet_t=np.loadtxt(path + 'naca0012_outlet_1211.dat',skiprows=1)
    ...
    #sampling
    xyu_s=np.loadtxt(path + 'naca0012_internal_1211.dat',skiprows=1)
    
    N_train=150
    idx = np.random.choice(len(xyu_s), N_train, replace=False)
    x_s = xyu_s[idx,0:1]
    y_s = xyu_s[idx,1:2]
    t_s = xyu_s[idx,2:3]
    p_s = xyu_s[idx,3:4]
...
#    v_train = np.concatenate((xyu_inlet[:,4:5],xyu_outlet_t[:,4:5],xyu_outlet_r[:,4:5],xyu_s[:,4:5]),axis=0)  
    ...
    xyu_int=np.loadtxt(path + 'naca0012_internal_1211.dat',skiprows=1)    
...
#                              x_iw, y_iw, t_iw, p_iw, u_iw, v_iw, nx_iw, ny_iw, \
#                              x_oo, y_oo, t_oo, p_oo, u_oo, v_oo, nx_oo, ny_oo, \
#                              xg_train, yg_train, tg_train, True)
# 
...
plt0, =plt.plot(xyu_wall[:,0:1],xyu_wall[:,1:2],'ok',linewidth=0,ms=2,label='MSE BC pts: 300',zorder=5)
...
plt0, =plt.plot(x_s,y_s,'og',linewidth=0,ms=3,label='MSE internal pts: 150 ',zorder=8)
...
plt.xlim(-1,2)
plt.ylim(-1,1)    
plt.savefig('./plot/mesh7.png', format='png',bbox_inches='tight', dpi=200)
plt.show()
