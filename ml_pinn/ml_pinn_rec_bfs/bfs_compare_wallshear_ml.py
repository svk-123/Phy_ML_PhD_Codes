...
Re=100  
fname_2=np.asarray([ ['bfs_%s_0'%Re,'bfs_%s_0_nodp_nodv_ws_x5'%Re] ])    
#fname_2.sort()
...
for jj in range(2):
        ...
        with open('./bfs_ml/%s/2591/wallShearStress'%(tmp[jj]), 'r') as infile:
...
xx=np.loadtxt('bottom_wall_x.dat')   

c=['g','g','b','r','r','m','darkorange','lime','pink','purple','peru','gold','olive','salmon','brown']          
plt.figure(figsize=(6,5),dpi=100)
plt.plot(xx[180:431],T[0][180:431],'o',mfc='None',mew=1.5,mec='k',ms=10,markevery=3,label='CFD')

for i in range(1,2):
    plt.plot(xx[180:431],T[i][180:431],'%s'%c[i],lw=2,label='%s'%mylabel[i])
