...
if __name__ == "__main__": 
    ...
    u_oo = xyu_outlet[:,3:4]
    v_oo = xyu_outlet[:,4:5]   
    nx_oo= xyu_outlet[:,5:6]
    ny_oo= xyu_outlet[:,6:7]

    
    xyu_upperwall=np.loadtxt(path + 'bfs_upperwall.dat',skiprows=1)
    
    x_upperwall = xyu_upperwall[:,0:1]
    y_upperwall = xyu_upperwall[:,1:2]
    p_upperwall = xyu_upperwall[:,2:3]
    u_upperwall = xyu_upperwall[:,3:4]
    v_upperwall = xyu_upperwall[:,4:5]       
    nx_upperwall= xyu_upperwall[:,5:6]
    ny_upperwall= xyu_upperwall[:,6:7]
    
    xyu_lowerwall=np.loadtxt(path + 'bfs_lowerwall.dat',skiprows=1)
    
    x_lowerwall = xyu_lowerwall[:,0:1]
    y_lowerwall = xyu_lowerwall[:,1:2]
    p_lowerwall = xyu_lowerwall[:,2:3]
    u_lowerwall = xyu_lowerwall[:,3:4]
    v_lowerwall = xyu_lowerwall[:,4:5]    
    nx_lowerwall= xyu_lowerwall[:,5:6]
    ny_lowerwall= xyu_lowerwall[:,6:7]

    #outlet-outlet
    x_wall  = np.concatenate((xyu_upperwall[:,0:1], xyu_lowerwall[:,0:1]),axis=0)
    y_wall  = np.concatenate((xyu_upperwall[:,1:2], xyu_lowerwall[:,1:2]),axis=0)
    p_wall  = np.concatenate((xyu_upperwall[:,2:3], xyu_lowerwall[:,2:3]),axis=0)
    u_wall  = np.concatenate((xyu_upperwall[:,3:4], xyu_lowerwall[:,3:4]),axis=0)    
    v_wall  = np.concatenate((xyu_upperwall[:,4:5], xyu_lowerwall[:,4:5]),axis=0) 
    nx_wall = np.concatenate((xyu_upperwall[:,5:6], xyu_lowerwall[:,5:6]),axis=0)    
    ny_wall = np.concatenate((xyu_upperwall[:,6:7], xyu_lowerwall[:,6:7]),axis=0) 
    
    #inlet-wall
    x_iw  = np.concatenate((xyu_inlet[:,0:1], xyu_upperwall[:,0:1], xyu_lowerwall[:,0:1]),axis=0)
    y_iw  = np.concatenate((xyu_inlet[:,1:2], xyu_upperwall[:,1:2], xyu_lowerwall[:,1:2]),axis=0)
    p_iw  = np.concatenate((xyu_inlet[:,2:3], xyu_upperwall[:,2:3], xyu_lowerwall[:,2:3]),axis=0)
    u_iw  = np.concatenate((xyu_inlet[:,3:4], xyu_upperwall[:,3:4], xyu_lowerwall[:,3:4]),axis=0)    
    v_iw  = np.concatenate((xyu_inlet[:,4:5], xyu_upperwall[:,4:5], xyu_lowerwall[:,4:5]),axis=0) 
    nx_iw = np.concatenate((xyu_inlet[:,5:6], xyu_upperwall[:,5:6], xyu_lowerwall[:,5:6]),axis=0)    
    ny_iw = np.concatenate((xyu_inlet[:,6:7], xyu_upperwall[:,6:7], xyu_lowerwall[:,6:7]),axis=0) 



    #sampling
    xyu_s=np.loadtxt(path + 'bfs_sample_x5_30.dat',skiprows=1)
...
#    v_train = np.concatenate((xyu_inlet[:,4:5],xyu_outlet_t[:,4:5],xyu_outlet_r[:,4:5],xyu_s[:,4:5]),axis=0)  
    ...
    #xyu_int=45206
    idx2 = np.random.choice(len(xyu_int_), 45206, replace=False)
    xyu_int=xyu_int_[idx2,:]
        
    # internal points with wall BC
    xg_train = np.concatenate((xyu_inlet[:,0:1],xyu_upperwall[:,0:1],xyu_lowerwall[:,0:1],xyu_outlet[:,0:1],xyu_int[:,0:1]),axis=0)
    yg_train = np.concatenate((xyu_inlet[:,1:2],xyu_upperwall[:,1:2],xyu_lowerwall[:,1:2],xyu_outlet[:,1:2],xyu_int[:,1:2]),axis=0)
...
#plt.xlim(-1,6)
#plt.ylim(-1,4)    
#plt.savefig('./plot/mesh6.png', format='png',bbox_inches='tight', dpi=200)
#plt.show()
