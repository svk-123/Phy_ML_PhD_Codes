    ...
    def train(self, nIter, lbfgs=False): 
        ...
        while(count < nIter):
            ...
            for i in range(total_batch):
                ...
                tf_dict = {self.x_tf: batch_x, self.y_tf: batch_y, self.u_tf: batch_u, self.v_tf: batch_v, self.p_tf: batch_p, self.tf_lr:lr,\
        ...
        if (lbfgs==True):
            ...
            tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.u_tf: self.u, self.v_tf: self.v, self.p_tf: batch_p,\
                       self.xg_tf: batch_x, self.yg_tf: batch_y}            
...
if __name__ == "__main__": 
    ...
    for ii in range(1):
        #x,y,Re,u,v
        with open('./data_file/cy_40_around_2533.pkl', 'rb') as infile:
            result = pickle.load(infile)
    ...
    xyu_io=np.loadtxt('./data_file/cy_inout_20.dat')
...
#    v_train = out_v[idx,:]
    ...
    #with wall BC
    x_train = np.concatenate((inp_x[idx,:],xyu[:,0:1], xyu_io[:,0:1]),axis=0)
    y_train = np.concatenate((inp_y[idx,:],xyu[:,1:2], xyu_io[:,1:2]),axis=0)
    u_train = np.concatenate((out_u[idx,:],xyu[:,2:3], xyu_io[:,2:3]),axis=0)
    v_train = np.concatenate((out_v[idx,:],xyu[:,3:4], xyu_io[:,3:4]),axis=0)
