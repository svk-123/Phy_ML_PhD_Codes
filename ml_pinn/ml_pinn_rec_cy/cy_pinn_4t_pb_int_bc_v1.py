...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, u, v, p, xb, yb, ub, vb, xg, yg,nx,ny, rst=False):
        ...
        self.ub_pred, self.vb_pred, self.pn_pred = self.net_NS2(self.xb_tf, self.yb_tf,self.nx_tf ,self.ny_tf )
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
        ...
        self.loss_2 = tf.reduce_mean(tf.square(self.ub_tf - self.ub_pred)) + \
                    ...
                    tf.reduce_mean(tf.square(self.pn_pred))
        ...
        while(count < nIter):
            ...
            for i in range(1):
                ...
                tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.u_tf: self.u, self.v_tf: self.v, self.p_tf: self.p, \
                           ...
                           self.tf_lr:lr, self.xg_tf: self.xg, self.yg_tf: self.yg, self.nx_tf:self.nx, self.ny_tf:self.ny}
...
if __name__ == "__main__": 
    ...
    xyu=np.loadtxt('./data_file/cy_wall_bc_nxny_100.dat',skiprows=1)
    
    #uvp inout
    xyu_io=np.loadtxt('./data_file/cy_inout_5555_4s_200_intp.dat')
...
#    yg_train = np.concatenate((y_train[:,:],yb_train[:,:],pinp_y[idx,:]),axis=0)
    ...
    # Training
    model = PhysicsInformedNN(x_train, y_train, u_train, v_train, p_train, xb_train, yb_train, ub_train, vb_train, xg_train, yg_train, nx,ny, False)
...
#plt.xlim(-0.5,2)
#plt.ylim(-0.5,1)    
plt.savefig('./plot/mesh_ppt.png', format='png',bbox_inches='tight', dpi=100)
plt.show()
