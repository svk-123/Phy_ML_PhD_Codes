...
if __name__ == "__main__": 
    ...
    xyu_io=np.loadtxt('./data_file/cy_inout_5555_4s_200.dat')
...
#    # only wall BC
    tb_train = np.concatenate((xyu[:,2:3], xyu_io[:,2:3]),axis=0)    
    xb_train = np.concatenate((xyu[:,0:1], xyu_io[:,0:1]),axis=0)
    yb_train = np.concatenate((xyu[:,1:2], xyu_io[:,1:2]),axis=0)
