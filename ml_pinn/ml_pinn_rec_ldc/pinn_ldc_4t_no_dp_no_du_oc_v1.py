...
if __name__ == "__main__": 
    ...
    #x,y,p,u,v
    xyu_inlet=np.loadtxt(path + 'ldc_int_cor.dat',skiprows=1)
    ...
    xyu_wall=np.loadtxt(path + 'ldc_fwall_cor.dat',skiprows=1)
    ...
    nx_iw = np.concatenate((xyu_inlet[:,4:5], xyu_wall[:,4:5]),axis=0)    
    ny_iw = np.concatenate((xyu_inlet[:,4:5], xyu_wall[:,4:5]),axis=0) 
...
#    v_train = np.concatenate((xyu_inlet[:,4:5],xyu_outlet_t[:,4:5],xyu_outlet_r[:,4:5],xyu_s[:,4:5]),axis=0)  
    ...
    xyu_int=np.loadtxt(path + 'ldc_internal_cor_left.dat',skiprows=1)    
