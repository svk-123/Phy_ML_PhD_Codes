...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, p, u, v, \
        ...
        self.xg_w_tf = tf.placeholder(tf.float32, shape=[None, self.xg_w.shape[1]])
        self.yg_w_tf = tf.placeholder(tf.float32, shape=[None, self.yg_w.shape[1]])        
        ...
        self.f_c_pred_a, self.f_u_pred_a, self.f_v_pred_a = self.net_NS2a(self.xg_w_tf, self.yg_w_tf)
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
        ...
        self.loss_2a = tf.reduce_mean(tf.square(self.f_c_pred_a)) + \
                    tf.reduce_mean(tf.square(self.f_u_pred_a)) + \
                    tf.reduce_mean(tf.square(self.f_v_pred_a))            
        ...
        self.loss = self.loss_0 + self.loss_1 + self.loss_2 + self.factor*self.loss_2a
        ...
        while(count < nIter):
            ...
            for i in range(1):
                ...
                tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.u_tf: self.u, self.v_tf: self.v, self.p_tf: self.p, \
                           ...
                           self.tf_lr:lr, self.xg_tf: self.xg, self.yg_tf: self.yg, self.xg_w_tf: self.xg_w, self.yg_w_tf: self.yg_w}
...
#    v_train = np.concatenate((xyu_inlet[:,4:5],xyu_outlet_t[:,4:5],xyu_outlet_r[:,4:5],xyu_s[:,4:5]),axis=0)  
    ...
    #etra weight
    xyu_w=np.loadtxt(path + 'ldc_corner.dat',skiprows=1)    
                
    # internal points with wall BC
    xg_train_w = xyu_w[:,0:1]
    yg_train_w = xyu_w[:,1:2]
        
    # Training
    model = PhysicsInformedNN(x_train, y_train, p_train, u_train, v_train, \
                              x_iw, y_iw, p_iw, u_iw, v_iw, nx_iw, ny_iw, \
                              x_oo, y_oo, p_oo, u_oo, v_oo, nx_oo, ny_oo, \
                              xg_train, yg_train,xg_train_w, yg_train_w)
...
#plt0, =plt.plot(xyu_w[:,0:1],xyu_w[:,1:2],'+b',linewidth=0,ms=2,label='Gov Eq. Res. pts: 400+ ',zorder=4)
#
####text-1
