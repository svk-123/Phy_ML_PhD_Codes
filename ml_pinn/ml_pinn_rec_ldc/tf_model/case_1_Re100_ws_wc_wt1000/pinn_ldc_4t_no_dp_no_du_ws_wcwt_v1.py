...
class PhysicsInformedNN:
    ...
    def __init__(self, x, y, p, u, v, \
        ...
        self.factor = tf.constant([1000.0], dtype=tf.float32)
        ...
        self.xc_tf = tf.placeholder(tf.float32, shape=[None, self.xc.shape[1]])        
        self.yc_tf = tf.placeholder(tf.float32, shape=[None, self.xc.shape[1]]) 
        self.pc_tf = tf.placeholder(tf.float32, shape=[None, self.xc.shape[1]]) 
        self.uc_tf = tf.placeholder(tf.float32, shape=[None, self.xc.shape[1]]) 
        self.vc_tf = tf.placeholder(tf.float32, shape=[None, self.xc.shape[1]]) 
        ...
        #corner
        self.uc_pred, self.vc_pred, self.pc_pred  = self.net_NS0a(self.xc_tf, self.yc_tf)
...
#                    tf.reduce_sum(tf.square(self.f_v_pred))
        ...
        self.loss_0a = tf.reduce_mean(tf.square(self.uc_tf - self.uc_pred)) + \
                    tf.reduce_mean(tf.square(self.vc_tf - self.vc_pred)) + \
                    tf.reduce_mean(tf.square(self.pc_tf - self.pc_pred))   
                    
        self.loss = self.loss_0 + self.loss_1 + self.loss_2 + self.factor*self.loss_0a
        ...
        while(count < nIter):
            ...
            for i in range(1):
                ...
                tf_dict = {self.x_tf: self.x, self.y_tf: self.y, self.u_tf: self.u, self.v_tf: self.v, self.p_tf: self.p, \
                           ...
                           self.xc_tf:self.xc, self.yc_tf:self.yc, self.pc_tf:self.pc, self.uc_tf:self.uc, self.vc_tf:self.vc }
...
#    x_train = np.concatenate((xyu_s[idx,0:1],xyu_s[idx,0:1]),axis=0)
#    y_train = np.concatenate((xyu_s[idx,1:2],xyu_s[idx,1:2]),axis=0)
#    p_train = np.concatenate((xyu_s[idx,2:3],xyu_s[idx,2:3]),axis=0)
#    u_train = np.concatenate((xyu_s[idx,3:4],xyu_s[idx,3:4]),axis=0)
#    v_train = np.concatenate((xyu_s[idx,4:5],xyu_s[idx,4:5]),axis=0)
