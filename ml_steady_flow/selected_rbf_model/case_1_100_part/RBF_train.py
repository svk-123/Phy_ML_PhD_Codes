...
if __name__ == "__main__":
    ...
    for i in range(len(ReNos)):
        data = load_data("../data/cavity_Re", ReNos[i],"_part" ".pkl")
        sampleNo = len(data[0])
        # input
...
#    centers_ori = rbflayer.get_weights()[0]
    ...
    # Compile model
    opt = Adam(lr=0.001, decay=1e-12)
    model.compile(loss='mean_squared_error', optimizer=opt)


    hist = model.fit(inputSamples, outputSamples, batch_size=64,validation_split=0.1,callbacks=[reduce_lr,e_stop,chkpt],epochs=1000, verbose=1)
