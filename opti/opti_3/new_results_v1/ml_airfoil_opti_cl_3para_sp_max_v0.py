...
path='./result_paper_v8/sp_3_tanh_cdc/'
tar_cl=1.2
tar_cd=0.05

reno=np.asarray([80000])/100000.
aoa=np.asarray([8])/14.
...
#mp2-relu
#foil=['naca0012','naca5014','naca5008','naca3012','naca2030','naca2010','naca1416']
...
for jj in range(len(foil)):
    ...
    def loss(para):
        ...
        my_inp=np.concatenate((reno,aoa, mypara/scaler),axis=0)
        ...
        #max-cl with cd constraint
        e= abs(pred_cd-tar_cd)*10.0 + (1.0/pred_cl)
        
        #e= max(0,(pred_cd-tar_cd)*10.0) + (1.0/pred_cl)
        ...
        if(my_counter == 0):
            ...
            fp_conv.write('Iter,   MSE,    Pred_cl, Pred Cd\n')
            
        my_counter = my_counter +1
        print ('Iter:', my_counter)
        
        fp_conv.write('%s %s %s %s\n'%(my_counter,e,pred_cl,pred_cd)) 
    ...
    res = minimize(loss, x0=p1, method = 'L-BFGS-B', bounds=mylimit, \
                   options={'disp': True, 'maxcor':200, 'ftol': 1e-16, \
                                     'eps': 1, 'maxfun': 200, \
                                     'maxiter': 200, 'maxls': 200})
