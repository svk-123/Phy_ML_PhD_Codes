...
def val_and_grad(x):
    with tf.GradientTape() as tape:
        ...
        out=model_mlp(x)
        #print(out)
        cd=out[0,0]*tf.constant([0.25])
        cl=out[0,1]*tf.constant([0.9])
 
        loss = tf.divide(cd,cl)
        #loss = tf.divide(cd,tf.pow(cl,tf.constant([1.5])))
        
        fp_conv.write('%f %f %f %f\n' %(loss.numpy()[0],cl.numpy()[0],cd.numpy()[0],1/loss.numpy()[0]))
...
for j in range(10):    
    ...
    fp_conv=open('./plot/conv_%s.dat'%fn,'w+')
    ...
    res = minimize(func, x0=p1, jac=True, method = 'L-BFGS-B', bounds=mylimit, \
    ...
    fp_res=open('./plot/res_%s.dat'%fn,'w')
